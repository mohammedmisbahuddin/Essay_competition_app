...
def generate_bca_registration_number(full_name, age):
    """
    Generate BCA format registration number: BCA + age(2) + name(2) + uuid(5)
    Format: BCA25JOA1B2C (12 characters total)
    """
    # Extract first 2 characters of name
    name_part = extract_name_part(full_name, 2)

    # Format age as 2 digits (clamp so a stray 3-digit age can't break the fixed width)
    age_str = f"{min(max(age, 0), 99):02d}" if age is not None else "00"

    # Generate 5-character UUID part
    uuid_part = str(uuid.uuid4())[:5].upper()

    # Combine: BCA + age + name + uuid
    registration_number = f"BCA{age_str}{name_part}{uuid_part}"

    # Ensure uniqueness (very unlikely to need retry with 5-char UUID), bounded to avoid a runaway loop
    attempts = 0
...
def generate_registration_number(full_name=None, age=None):
    ...
    Uses new BCA format if name and age are provided, otherwise falls back to original format
    """
